...
**Urutan section** (jangan diubah tanpa alasan, ini alur konversinya): header sticky → hero → 3 keunggulan (`#keunggulan`) → 3 kartu kelas (`#kelas`) → 3 testimoni (`#testimoni`) → panel CTA penutup → footer → bar CTA melayang khusus mobile.
...
Alur menambah foto baru: unduh via `commons.wikimedia.org/w/api.php` (Unsplash memblokir unduhan otomatis; endpoint `napi` Wikimedia butuh URL thumb persis dari API — lebar sembarang membalas HTTP 400), lalu potong dan kompres dengan Pillow, sekitar 900×675 untuk kartu dan 1600×1067 untuk hero, JPEG kualitas 82 progresif. Total `assets/` sekarang ± 780 KB; jaga di kisaran itu.

## Isi yang masih contoh

Harga, jumlah sesi, nama & kutipan testimoni, "sisa 6 kursi", rating 4,9 dari 512 murid, dan alamat `halo@dapurrina.id` semuanya karangan untuk demo. Perlakukan sebagai placeholder, bukan fakta yang harus dipertahankan konsisten.
